...
class League(commands.Cog):
        ...
        with self.db._lock:
            try:
                ...
                cursor.execute(
                    ...
                    LEFT JOIN league_aliases la
                        ON lp.user_id = la.user_id
                    WHERE lp.user_id = ?
                       OR LOWER(la.alias) = LOWER(?)
                    LIMIT 1
                    ''',
    ...
    @commands.command(
        name="aliasy",
        description="Pokazuje listę wszystkich aliasów w systemie"
    )
        ...
        with self.db._lock:
            try:
                ...
                cursor.execute(
                    ...
                    JOIN league_profiles lp
                        ON la.user_id = lp.user_id
                    '''
                )
    ...
    async def dodaj(self, ctx, alias_lub_mention: str, nick_z_tagiem: str):
        """Pozwala dodawać użytkowników do systemu (LoL)"""
    ...
    @commands.hybrid_command(
        name="rank",
        description="Sprawdza rangę gracza na OP.GG"
    )
    async def rank(self, ctx, cel: str):
        ...
        if not self.bot.session:
            await ctx.send("Błąd sesji HTTP.")
            return
        ...
        if not data.get("found"):
            await ctx.send(
                f"Nie znaleziono danych rankingowych dla {target}."
            )
            return
    ...
    @commands.hybrid_command(
        name="mastery",
        description="Pokazuje TOP 3 postacie pod względem punktów maestrii"
    )
    async def mastery(self, ctx, cel: str):
        await ctx.defer()
        logger.info(f"Użytkownik {ctx.author} sprawdza mastery: {cel}")
        ...
        if "#" not in target:
            await ctx.send(
                "Podaj `Nick#Tag` lub dodaj gracza za pomocą `!dodaj`."
            )
            return
        ...
        if not data:
            await ctx.send(
                "Nie znaleziono danych o mastery. Odśwież profil na OP.GG."
            )
            return
