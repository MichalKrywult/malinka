...
class ReminderModal(ui.Modal, title='Dodaj Przypomnienie'):
    data_input = ui.TextInput(
        ...
        min_length=5,
        max_length=5,
        default=datetime.now().strftime("%d.%m")
    )
        ...
        try:
            ...
            with self.db._lock:
                self.db.conn.execute(
                    """
                    INSERT INTO reminder (user_id, content, remind_at)
                    VALUES (?, ?, ?)
                    """,
...
class ReminderActionView(ui.View):
    ...
    async def snooze(self, interaction: discord.Interaction, button: ui.Button):
        ...
        await interaction.response.edit_message(
            content=f"Odłożono: **{self.content}** o 15 minut.",
            view=None
        )
    ...
    async def done(self, interaction: discord.Interaction, button: ui.Button):
        await interaction.response.edit_message(
            content=f"Zrobione: **{self.content}**",
            view=None
        )
        self.stop()
            ...
            try:
                ...
                for rem_id, user_id, content in pending:
                    ...
                    try:
                        ...
                        await user.send(
                            f"**Przypomnienie:** {content}",
                            view=view
                        )

                        cursor.execute(
                            "UPDATE reminder SET is_sent = 1 WHERE id = ?",
                            (rem_id,)
                        )
                        ...
                        if str(user_id) == str(OWNER_DISCORD_ID):
                            try:
                                ...
                                logger.warning(
                                    f"Nieudane wysłanie na Telegram: {e}"
                                )

                    except Exception as e:
                        logger.error(
                            f"Błąd wysyłania DM do {user_id}: {e}"
                        )
                ...
                logger.exception(
                    f"Błąd bazy danych w pętli przypomnień: {e}"
                )


async def setup(bot):
    await bot.add_cog(
        Reminders(bot, bot.db)
    )
